...
use listeners::{
    EContext, ExprLexer, ExprListener, ExprParser, ExprParserContextType, ExprTreeWalker,
    ExprVisitor, SContext, exprparser,
};
...
impl<'i> ParseTreeVisitor<'i, ExprParserContextType> for EvaluatorParser {
    fn visit_terminal(&mut self, node: &antlr4rust::tree::TerminalNode<'i, ExprParserContextType>) {
        ...
        if node.symbol.get_token_type() == exprparser::Expr_INT {
            ...
            self.evaluator
                .stack
                .push(node.get_text().parse::<i64>().unwrap());
...
impl<'i> ExprVisitor<'i> for EvaluatorParser {
    fn visit_s(&mut self, ctx: &SContext<'i>) {
        self.visit_children(ctx)
    }

    fn visit_e(&mut self, ctx: &EContext<'i>) {
        self.visit_children(ctx)
    }
}
...
fn main() {
    ...
    parser.add_parse_listener(Box::new(EvaluatorParser {
        evaluator: Default::default(),
    }));
